...
public class NumericUtilities {
    ...
    public static Integer randomInteger(String range) {
        String[] tokens = StringUtilities.split(range, ",");
        String startSymbol = StringUtilities.firstCharacter(tokens[0]);
        String endSymbol = StringUtilities.lastCharacter(tokens[1]);
    ...
    public static long parseLong(String number) {
        if (StringUtilities.isBlank(number)) {
            return 0;
        }
